...
impl<T, const WIDTH: usize> Histogram<T, WIDTH> {
    ...
    /// Returns the count for a specific bucket across all slots.
    fn bucket_total(&self, bucket: usize) -> u64 {
        self.aggregate_buckets[bucket]
    }
...
mod tests {
    ...
    fn test_record_and_total() {
        ...
        assert_eq!(
            hist.get_bucket(Histogram::<()>::calculate_bucket_uncached(10)),
            1
        );
        assert_eq!(
    ...
    fn test_advance_capacity_stays_constant() {
        ...
        for i in 3..10 {
            ...
            assert_eq!(
                hist.capacity(),
                3,
                "capacity grew unexpectedly at iteration {}",
                i
            );
            assert_eq!(hist.active_slot_count(), 3);
        }
